...
printf "mode\tqid\trepeat\tstatus\trc\tseconds\twall_time_ns\tbuf_hit\truntime_sync\truntime_async\tdisk_sync\tdisk_async\tlogical_ns\tsql\trun_root\n" > "$COMBINED"
...
awk -F '\t' '
  ...
  END {
    print "qid\trepeat\tno_status\tdsm_status\tno_seconds\tdsm_seconds\tno_wall_time_ns\tdsm_wall_time_ns\tno_logical_ns\tdsm_logical_ns\tlogical_reduction_pct\tno_disk_sync\tdsm_runtime_sync\tdsm_disk_sync\tno_disk_async\tdsm_runtime_async\tdsm_disk_async\tbuf_no\tbuf_dsm\tno_run_root\tdsm_run_root"
    for (key in rows) {
      no = rows[key, "no_dsm"]
      dsm = rows[key, "dsm"]
      if (no == "" || dsm == "") { continue }
      ...
      split(no, n, "\t")
      split(dsm, d, "\t")
      reduction = (n[13] > 0) ? (100.0 * (n[13] - d[13]) / n[13]) : 0
      printf "%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%.3f\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\n", \
        k[1], k[2], n[4], d[4], n[6], d[6], n[7], d[7], n[13], d[13], reduction, \
        n[11], d[9], d[11], n[12], d[10], d[12], n[8], d[8], n[15], d[15]
    }
  }
