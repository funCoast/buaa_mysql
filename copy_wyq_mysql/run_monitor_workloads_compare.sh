...
# It runs four isolated cases:
#   TPCH_NO_DSM, TPCH_DSM, TPCC_NO_DSM, TPCC_DSM,
#   MICRO_SCAN_NO_DSM, MICRO_SCAN_DSM,
#   MICRO_RANDOM_LOOKUP_NO_DSM, MICRO_RANDOM_LOOKUP_DSM,
#   MICRO_RANDOM_UPDATE_NO_DSM, MICRO_RANDOM_UPDATE_DSM,
#   MICRO_SEQ_WRITE_NO_DSM, MICRO_SEQ_WRITE_DSM
...
MICRO_RANDOM_UPDATE_ROWS_PER_PAGE="${MICRO_RANDOM_UPDATE_ROWS_PER_PAGE:-4}"
MICRO_RANDOM_UPDATE_PASSES="${MICRO_RANDOM_UPDATE_PASSES:-3}"
MICRO_RANDOM_UPDATE_WARMUP_PASSES="${MICRO_RANDOM_UPDATE_WARMUP_PASSES:-0}"
MICRO_RANDOM_UPDATE_STRIDE="${MICRO_RANDOM_UPDATE_STRIDE:-997}"
MICRO_RANDOM_UPDATE_SINGLE_TXN="${MICRO_RANDOM_UPDATE_SINGLE_TXN:-1}"

MICRO_SEQ_WRITE_DB="${MICRO_SEQ_WRITE_DB:-micro_seq_write_monitor}"
MICRO_SEQ_WRITE_INITIAL_ROWS="${MICRO_SEQ_WRITE_INITIAL_ROWS:-12000}"
MICRO_SEQ_WRITE_INSERT_ROWS="${MICRO_SEQ_WRITE_INSERT_ROWS:-6000}"
MICRO_SEQ_WRITE_WARMUP_ROWS="${MICRO_SEQ_WRITE_WARMUP_ROWS:-0}"
MICRO_SEQ_WRITE_SINGLE_TXN="${MICRO_SEQ_WRITE_SINGLE_TXN:-1}"

RUN_TPCH="${RUN_TPCH:-1}"
RUN_TPCC="${RUN_TPCC:-1}"
RUN_MICRO_SCAN="${RUN_MICRO_SCAN:-1}"
RUN_MICRO_RANDOM="${RUN_MICRO_RANDOM:-1}"
RUN_MICRO_RANDOM_UPDATE="${RUN_MICRO_RANDOM_UPDATE:-0}"
RUN_MICRO_SEQ_WRITE="${RUN_MICRO_SEQ_WRITE:-0}"
RUN_MICRO="${RUN_MICRO:-}"
if [[ -n "$RUN_MICRO" ]]; then
  RUN_MICRO_SCAN="$RUN_MICRO"
  RUN_MICRO_RANDOM="$RUN_MICRO"
  RUN_MICRO_RANDOM_UPDATE="$RUN_MICRO"
  RUN_MICRO_SEQ_WRITE="$RUN_MICRO"
fi
...
Environment knobs:
  ...
  MICRO_RANDOM_UPDATE_SINGLE_TXN=0/1 default: 1
  MICRO_SEQ_WRITE_SINGLE_TXN=0/1 default: 1
  ...
  MICRO_RANDOM_UPDATE_WARMUP_PASSES=N default: 0
  MICRO_SEQ_WRITE_WARMUP_ROWS=N default: 0
  COST_BUF_NS=N                default: 100
  COST_DSM_NS=N                default: 300
  COST_DISK_NS=N               default: 30000
  RUN_TPCH=0/RUN_TPCC=0/RUN_MICRO_SCAN=0/RUN_MICRO_RANDOM=0/RUN_MICRO_RANDOM_UPDATE=0/RUN_MICRO_SEQ_WRITE=0
...
write_run_config() {
  local out="$RUN_ROOT/config.txt"
  {
    ...
    printf "MICRO_RANDOM_UPDATE_PASSES=%s\n" "$MICRO_RANDOM_UPDATE_PASSES"
    printf "MICRO_RANDOM_UPDATE_STRIDE=%s\n" "$MICRO_RANDOM_UPDATE_STRIDE"
    printf "MICRO_RANDOM_UPDATE_SINGLE_TXN=%s\n" "$MICRO_RANDOM_UPDATE_SINGLE_TXN"
    printf "MICRO_SEQ_WRITE_INITIAL_ROWS=%s\n" "$MICRO_SEQ_WRITE_INITIAL_ROWS"
    printf "MICRO_SEQ_WRITE_WARMUP_ROWS=%s\n" "$MICRO_SEQ_WRITE_WARMUP_ROWS"
    printf "MICRO_SEQ_WRITE_INSERT_ROWS=%s\n" "$MICRO_SEQ_WRITE_INSERT_ROWS"
    printf "MICRO_SEQ_WRITE_SINGLE_TXN=%s\n" "$MICRO_SEQ_WRITE_SINGLE_TXN"
    printf "RUN_TPCH=%s\n" "$RUN_TPCH"
    printf "RUN_TPCC=%s\n" "$RUN_TPCC"
    printf "RUN_MICRO_SCAN=%s\n" "$RUN_MICRO_SCAN"
    printf "RUN_MICRO_RANDOM=%s\n" "$RUN_MICRO_RANDOM"
    printf "RUN_MICRO_RANDOM_UPDATE=%s\n" "$RUN_MICRO_RANDOM_UPDATE"
    printf "RUN_MICRO_SEQ_WRITE=%s\n" "$RUN_MICRO_SEQ_WRITE"
...
load_micro_random_update() {
  local db="$1"
  local saved_pages="$MICRO_RANDOM_PAGES"
  local saved_rows_per_page="$MICRO_RANDOM_ROWS_PER_PAGE"
  MICRO_RANDOM_PAGES="$MICRO_RANDOM_UPDATE_PAGES"
  MICRO_RANDOM_ROWS_PER_PAGE="$MICRO_RANDOM_UPDATE_ROWS_PER_PAGE"
  load_micro_random_lookup "$db"
  MICRO_RANDOM_PAGES="$saved_pages"
  MICRO_RANDOM_ROWS_PER_PAGE="$saved_rows_per_page"
  mysql_cmd "$db" -e "ALTER TABLE hot ADD COLUMN marker INT NOT NULL DEFAULT 0;" >/dev/null
  mysql_cmd "$db" -e "ANALYZE TABLE hot;" >/dev/null
}

run_micro_random_update_workload() {
  local db="$1" out="$2"
  local sql="$RUN_ROOT/${db}_random_update.sql"
  : > "$out"
  : > "$sql"
  {
    echo "USE \`$db\`;"
    if [[ "$MICRO_RANDOM_UPDATE_SINGLE_TXN" == "1" ]]; then
      echo "SET autocommit=0;"
      echo "START TRANSACTION;"
    else
      echo "SET autocommit=1;"
    fi
    for pass in $(seq 1 "$MICRO_RANDOM_UPDATE_PASSES"); do
      for i in $(seq 0 $(( MICRO_RANDOM_UPDATE_PAGES - 1 ))); do
        local page=$(( (i * MICRO_RANDOM_UPDATE_STRIDE) % MICRO_RANDOM_UPDATE_PAGES ))
        local id=$(( page * MICRO_RANDOM_UPDATE_ROWS_PER_PAGE + 1 ))
        echo "UPDATE hot FORCE INDEX(PRIMARY) SET marker = marker + 1 WHERE id = $id;"
      done
    done
    if [[ "$MICRO_RANDOM_UPDATE_SINGLE_TXN" == "1" ]]; then
      echo "COMMIT;"
...
SQL
  ...
  if [[ "$MICRO_SEQ_WRITE_INITIAL_ROWS" -gt 0 ]]; then
    mysql_cmd "$db" -e "INSERT INTO hot (marker, pad01, pad02, pad03, pad04) VALUES (0, REPEAT('A',255), REPEAT('B',255), REPEAT('C',255), REPEAT('D',255));" >/dev/null
    local cur=1
    while [[ $cur -lt "$MICRO_SEQ_WRITE_INITIAL_ROWS" ]]; do
      mysql_cmd "$db" -e "INSERT INTO hot (marker, pad01, pad02, pad03, pad04) SELECT marker, pad01, pad02, pad03, pad04 FROM hot LIMIT $cur;" >/dev/null
      cur=$(( cur * 2 ))
    done
    mysql_cmd "$db" -e "DELETE FROM hot WHERE id > $MICRO_SEQ_WRITE_INITIAL_ROWS;" >/dev/null
  fi
  mysql_cmd "$db" -e "ANALYZE TABLE hot;" >/dev/null
}

run_micro_seq_write_workload() {
  local db="$1" out="$2"
  local sql="$RUN_ROOT/${db}_seq_write.sql"
  : > "$out"
  : > "$sql"
  {
    echo "USE \`$db\`;"
    if [[ "$MICRO_SEQ_WRITE_SINGLE_TXN" == "1" ]]; then
      echo "SET autocommit=0;"
      echo "START TRANSACTION;"
    else
      echo "SET autocommit=1;"
    fi
    for i in $(seq 1 "$MICRO_SEQ_WRITE_INSERT_ROWS"); do
      echo "INSERT INTO hot (marker, pad01, pad02, pad03, pad04) VALUES ($i, REPEAT('W',255), REPEAT('X',255), REPEAT('Y',255), REPEAT('Z',255));"
    done
    if [[ "$MICRO_SEQ_WRITE_SINGLE_TXN" == "1" ]]; then
      echo "COMMIT;"
    fi
...
run_workload_once() {
  ...
  case "$workload" in
    ...
    MICRO_RANDOM_UPDATE) run_micro_random_update_workload "$MICRO_RANDOM_UPDATE_DB" "${out_prefix}_micro_random_update.out" ;;
    MICRO_SEQ_WRITE) run_micro_seq_write_workload "$MICRO_SEQ_WRITE_DB" "${out_prefix}_micro_seq_write.out" ;;
...
run_warmup_if_needed() {
  ...
  case "$workload" in
    ...
    MICRO_RANDOM_LOOKUP)
      ...
      ;;
    MICRO_RANDOM_UPDATE)
      [[ "$MICRO_RANDOM_UPDATE_WARMUP_PASSES" -gt 0 ]] || return 0
      local saved_update_passes="$MICRO_RANDOM_UPDATE_PASSES"
      local rc
      MICRO_RANDOM_UPDATE_PASSES="$MICRO_RANDOM_UPDATE_WARMUP_PASSES"
      run_micro_random_update_workload "$MICRO_RANDOM_UPDATE_DB" "${out_prefix}_warmup_micro_random_update.out"
      rc=$?
      MICRO_RANDOM_UPDATE_PASSES="$saved_update_passes"
      return "$rc"
      ;;
    MICRO_SEQ_WRITE)
      [[ "$MICRO_SEQ_WRITE_WARMUP_ROWS" -gt 0 ]] || return 0
      local saved_insert_rows="$MICRO_SEQ_WRITE_INSERT_ROWS"
      local rc
      MICRO_SEQ_WRITE_INSERT_ROWS="$MICRO_SEQ_WRITE_WARMUP_ROWS"
      run_micro_seq_write_workload "$MICRO_SEQ_WRITE_DB" "${out_prefix}_warmup_micro_seq_write.out"
      rc=$?
      MICRO_SEQ_WRITE_INSERT_ROWS="$saved_insert_rows"
      return "$rc"
      ;;
    *) echo "unknown workload: $workload" >&2; return 2 ;;
  esac
...
run_case() {
  ...
  case "$workload" in
    ...
    MICRO_RANDOM_UPDATE) load_micro_random_update "$MICRO_RANDOM_UPDATE_DB" ;;
    MICRO_SEQ_WRITE) load_micro_seq_write "$MICRO_SEQ_WRITE_DB" ;;
    *) echo "unknown workload: $workload" >&2; return 2 ;;
  esac
  case "$workload" in
...
if [[ "$RUN_MICRO_RANDOM_UPDATE" == "1" ]]; then
  run_case MICRO_RANDOM_UPDATE no_dsm
fi
if [[ "$RUN_MICRO_SEQ_WRITE" == "1" ]]; then
  run_case MICRO_SEQ_WRITE no_dsm
fi
...
if [[ "$RUN_MICRO_RANDOM_UPDATE" == "1" ]]; then
  run_case MICRO_RANDOM_UPDATE dsm
fi
if [[ "$RUN_MICRO_SEQ_WRITE" == "1" ]]; then
  run_case MICRO_SEQ_WRITE dsm
fi
stop_dsm_runtime
