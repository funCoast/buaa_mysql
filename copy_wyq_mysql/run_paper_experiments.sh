  ...
  for pages in $PAPER_RANDOM_PAGES_LIST; do
    for repeat in $(seq 1 "$PAPER_REPEATS"); do
      run_one "micro_random" "$MICRO_RANDOM_OUT" "pages=${pages}_bp=5M_dsm=${PAPER_DSM_BYTES}" "$repeat" \
        ...
        MICRO_RANDOM_PAGES="$pages" MICRO_RANDOM_ROWS_PER_PAGE=4 MICRO_RANDOM_WARMUP_PASSES="$PAPER_MICRO_WARMUP_PASSES" MICRO_RANDOM_PASSES=8 MICRO_RANDOM_STRIDE=997
    done
  done
fi
  ...
  for dsm_bytes in $PAPER_DSM_BYTES_LIST; do
    for repeat in $(seq 1 "$PAPER_REPEATS"); do
      run_one "dsm_capacity" "$DSM_CAPACITY_OUT" "pages=6000_bp=5M_dsm=${dsm_bytes}" "$repeat" \
        ...
        MICRO_RANDOM_PAGES=6000 MICRO_RANDOM_ROWS_PER_PAGE=4 MICRO_RANDOM_WARMUP_PASSES="$PAPER_MICRO_WARMUP_PASSES" MICRO_RANDOM_PASSES=8 MICRO_RANDOM_STRIDE=997
    done
  done
fi
