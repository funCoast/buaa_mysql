...
PAPER_DSM_BYTES="${PAPER_DSM_BYTES:-4294967296}"
PAPER_FIL_READ_CACHE_MAX_PAGES="${PAPER_FIL_READ_CACHE_MAX_PAGES:-1048576}"
PAPER_MICRO_WARMUP_PASSES="${PAPER_MICRO_WARMUP_PASSES:-2}"
PAPER_MICRO_RANDOM_BATCH_SIZE="${PAPER_MICRO_RANDOM_BATCH_SIZE:-1}"
PAPER_SCAN_WARMUP_REPEATS="${PAPER_SCAN_WARMUP_REPEATS:-2}"
PAPER_TPCH_WARMUP_REPEATS="${PAPER_TPCH_WARMUP_REPEATS:-2}"
...
PAPER_DSM_BYTES_LIST="${PAPER_DSM_BYTES_LIST:-16777216 67108864 134217728 268435456 536870912 1073741824 2147483648 4294967296}"
...
write_top_config() {
  {
    ...
    printf "PAPER_MICRO_RANDOM_BATCH_SIZE=%s\n" "$PAPER_MICRO_RANDOM_BATCH_SIZE"
...
if [[ "$PAPER_RUN_MICRO_RANDOM" == "1" ]]; then
  for pages in $PAPER_RANDOM_PAGES_LIST; do
    for repeat in $(seq 1 "$PAPER_REPEATS"); do
      run_one "micro_random" "$MICRO_RANDOM_OUT" "pages=${pages}_bp=5M_dsm=${PAPER_DSM_BYTES}_batch=${PAPER_MICRO_RANDOM_BATCH_SIZE}" "$repeat" \
        RUN_TPCH=0 RUN_TPCC=0 RUN_MICRO_SCAN=0 RUN_MICRO_RANDOM=1 \
        MYSQL_BP_SIZE=5M DSM_CACHE_BYTES_PER_NODE="$PAPER_DSM_BYTES" FIL_READ_CACHE_MAX_PAGES="$PAPER_FIL_READ_CACHE_MAX_PAGES" \
        MICRO_RANDOM_PAGES="$pages" MICRO_RANDOM_ROWS_PER_PAGE=4 MICRO_RANDOM_WARMUP_PASSES="$PAPER_MICRO_WARMUP_PASSES" MICRO_RANDOM_PASSES=8 MICRO_RANDOM_STRIDE=997 MICRO_RANDOM_BATCH_SIZE="$PAPER_MICRO_RANDOM_BATCH_SIZE"
    done
  done
fi
  ...
  for pages in $PAPER_RANDOM_UPDATE_PAGES_LIST; do
    for repeat in $(seq 1 "$PAPER_REPEATS"); do
      run_one "micro_random_update" "$MICRO_RANDOM_UPDATE_OUT" "pages=${pages}_bp=5M_dsm=${PAPER_DSM_BYTES}" "$repeat" \
        ...
        MYSQL_BP_SIZE=5M DSM_CACHE_BYTES_PER_NODE="$PAPER_DSM_BYTES" FIL_READ_CACHE_MAX_PAGES="$PAPER_FIL_READ_CACHE_MAX_PAGES" \
        MICRO_RANDOM_UPDATE_PAGES="$pages" MICRO_RANDOM_UPDATE_ROWS_PER_PAGE=4 MICRO_RANDOM_UPDATE_WARMUP_PASSES="$PAPER_MICRO_WARMUP_PASSES" MICRO_RANDOM_UPDATE_PASSES="$PAPER_RANDOM_UPDATE_PASSES" MICRO_RANDOM_UPDATE_STRIDE=997 MICRO_RANDOM_UPDATE_SINGLE_TXN=1
    done
  done
fi
...
if [[ "$PAPER_RUN_DSM_CAPACITY" == "1" ]]; then
  for dsm_bytes in $PAPER_DSM_BYTES_LIST; do
    for repeat in $(seq 1 "$PAPER_REPEATS"); do
      run_one "dsm_capacity" "$DSM_CAPACITY_OUT" "pages=6000_bp=5M_dsm=${dsm_bytes}_batch=${PAPER_MICRO_RANDOM_BATCH_SIZE}" "$repeat" \
        RUN_TPCH=0 RUN_TPCC=0 RUN_MICRO_SCAN=0 RUN_MICRO_RANDOM=1 \
        MYSQL_BP_SIZE=5M DSM_CACHE_BYTES_PER_NODE="$dsm_bytes" FIL_READ_CACHE_MAX_PAGES="$PAPER_FIL_READ_CACHE_MAX_PAGES" \
        MICRO_RANDOM_PAGES=6000 MICRO_RANDOM_ROWS_PER_PAGE=4 MICRO_RANDOM_WARMUP_PASSES="$PAPER_MICRO_WARMUP_PASSES" MICRO_RANDOM_PASSES=8 MICRO_RANDOM_STRIDE=997 MICRO_RANDOM_BATCH_SIZE="$PAPER_MICRO_RANDOM_BATCH_SIZE"
    done
  done
fi
...
if [[ "$PAPER_RUN_BP_SENSITIVITY" == "1" ]]; then
  for bp_size in $PAPER_BP_SIZE_LIST; do
    for repeat in $(seq 1 "$PAPER_REPEATS"); do
      run_one "bp_sensitivity" "$BP_SENSITIVITY_OUT" "pages=6000_bp=${bp_size}_dsm=${PAPER_DSM_BYTES}_batch=${PAPER_MICRO_RANDOM_BATCH_SIZE}" "$repeat" \
        RUN_TPCH=0 RUN_TPCC=0 RUN_MICRO_SCAN=0 RUN_MICRO_RANDOM=1 \
        MYSQL_BP_SIZE="$bp_size" DSM_CACHE_BYTES_PER_NODE="$PAPER_DSM_BYTES" FIL_READ_CACHE_MAX_PAGES="$PAPER_FIL_READ_CACHE_MAX_PAGES" \
        MICRO_RANDOM_PAGES=6000 MICRO_RANDOM_ROWS_PER_PAGE=4 MICRO_RANDOM_WARMUP_PASSES="$PAPER_MICRO_WARMUP_PASSES" MICRO_RANDOM_PASSES=8 MICRO_RANDOM_STRIDE=997 MICRO_RANDOM_BATCH_SIZE="$PAPER_MICRO_RANDOM_BATCH_SIZE"
    done
  done
fi
  ...
  for pages in $PAPER_SCAN_PAGES_LIST; do
    for repeat in $(seq 1 "$PAPER_REPEATS"); do
      run_one "micro_scan" "$MICRO_SCAN_OUT" "hot_pages=${pages}_bp=5M_dsm=${PAPER_DSM_BYTES}" "$repeat" \
        ...
        MYSQL_BP_SIZE=5M DSM_CACHE_BYTES_PER_NODE="$PAPER_DSM_BYTES" FIL_READ_CACHE_MAX_PAGES="$PAPER_FIL_READ_CACHE_MAX_PAGES" \
        MICRO_SCAN_HOT_PAGES="$pages" MICRO_SCAN_WARMUP_REPEATS="$PAPER_SCAN_WARMUP_REPEATS" MICRO_SCAN_QUERY_REPEATS=8
    done
  done
fi
...
if [[ "$PAPER_RUN_TPCH" == "1" ]]; then
  for repeat in $(seq 1 "$PAPER_REPEATS"); do
    run_one "tpch" "$TPCH_OUT" "sf1_q1-3-6-12-14-19_bp=5M_dsm=${PAPER_DSM_BYTES}" "$repeat" \
      ...
      MYSQL_BP_SIZE=5M DSM_CACHE_BYTES_PER_NODE="$PAPER_DSM_BYTES" FIL_READ_CACHE_MAX_PAGES="$PAPER_FIL_READ_CACHE_MAX_PAGES" \
      TPCH_DATASET=sf1 TPCH_QIDS="$PAPER_TPCH_QIDS" TPCH_WARMUP_REPEATS="$PAPER_TPCH_WARMUP_REPEATS" TPCH_QUERY_REPEATS="$PAPER_TPCH_QUERY_REPEATS" TPCH_QUERY_TIMEOUT_SEC="$PAPER_TPCH_QUERY_TIMEOUT_SEC"
  done
fi
