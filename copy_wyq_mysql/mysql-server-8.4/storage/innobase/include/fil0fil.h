...
/** Whether cache-source monitor is enabled. Controlled by
FIL_CACHE_MONITOR_ENABLE. */
bool enabled();

/** Returns true if this page belongs to a user tablespace page that we want to
track in the cache-source monitor. */
bool is_cacheable_page(const page_id_t &page_id);

/** Record that a cacheable page access was satisfied directly from buffer
pool. */
void record_buf_hit(const page_id_t &page_id);

/** Record that a cacheable page access was satisfied from DSM runtime.
@param[in] page_id   page identifier
@param[in] sync      true for foreground sync reads, false for async/background
*/
void record_runtime_hit(const page_id_t &page_id, bool sync);

/** Record that a cacheable page access fell through to physical disk I/O.
@param[in] page_id   page identifier
@param[in] sync      true for foreground sync reads, false for async/background
*/
...
#ifndef UNIV_HOTBACKUP
/** Read-through cache: a layer between buffer pool and disk.
 *  Declared in fil0fil.cc, used by buf0lru.cc for L2 cache on eviction. */
namespace fil_read_cache {

/** Try to get page from cache.
@param[in]  page_id  page identifier
@param[in]  len      page length in bytes
@param[out] buf      buffer to copy cached page into
@return true if found and copied to buf */
bool get(const page_id_t &page_id, ulint len, void *buf);

/** Put page into cache (after disk read or on LRU eviction).
@param[in]  page_id  page identifier
@param[in]  len      page length in bytes
@param[in]  buf      page content to cache */
