...
static inline void buf_LRU_remove_block(buf_page_t *bpage) {
  ...
  // std::cout << "Removing block " << bpage->id.space() << ":"
  //           << bpage->id.page_no() << " from LRU list\n";
  // std::string root_dir = "/workspace/mysql_install/runtime_data/";
  // std::string filename = root_dir + std::to_string(bpage->id.space()) + "_" +
  //                        std::to_string(bpage->id.page_no()) + ".page";
  // std::string logname = root_dir + "data_record.log";

  // // buf_block_t *block = buf_block_hash_get(buf_pool, bpage->id);
  // // const byte *frame = buf_page_get_frame(bpage);
  // std::ofstream logfile(filename, std::ios::app | std::ios::binary);
  // std::ofstream record_log(logname, std::ios::app);
  // if (!logfile) {
  //   // 你可以用 perror / ut_error 等
  //   std::cerr << "open failed: " << filename << "\n";
  // } else {
  //   // 1) 先写一段可读header（可选）
  //   record_log << "Removing block from LRU list: "
  //              << "Space ID: " << bpage->id.space()
  //              << ", Page No: " << bpage->id.page_no() << "\n";
  //   // 这是压缩页：数据在 bpage->zip.data，长度通常用 bpage->size.physical()
  //   if (bpage->zip.data != nullptr) {
  //     const byte *frame = static_cast<const byte *>(bpage->zip.data);
  //     ulint nbytes = bpage->size.physical();  // 或你确认的正确长度
  //     logfile.write(reinterpret_cast<const char *>(frame),
  //                   static_cast<std::streamsize>(nbytes));
  //   } else {
  //     // 非压缩页：bpage 实际就是 buf_block_t 的第一个成员
  //     buf_block_t *block = reinterpret_cast<buf_block_t *>(bpage);
  //     const byte *frame = block->frame;
  //     ulint nbytes = bpage->size.logical();  // 通常 16KiB
  //     std::cout << "Non-compressed page, writing " << nbytes << " bytes\n";
  //     logfile.write(reinterpret_cast<const char *>(frame),
  //                   static_cast<std::streamsize>(nbytes));
  //   }
  //   // 3) 可选：加一个分隔符，方便多次 append 后解析
  //   record_log << "finish" << "\n";
  //   logfile.close();
  //   record_log.close();
  // }
...
bool buf_LRU_free_page(buf_page_t *bpage, bool zip) {
  ...
  if (!buf_page_can_relocate(bpage)) {
    ...
    /*
1) buffer fixed（被固定引用）
含义：有线程正在使用这个页（比如拿着它的 latch，或正在对记录做读写/遍历），它的
page frame 不能随便换地址或释放。
否则会出现：
线程还拿着旧指针/旧 frame，页被挪走后变成悬空指针
数据结构访问越界/读到别的页内容
latch / hazard pointer 语义被破坏
2) I/O-fixed（正在进行 IO）
含义：这个页正在被异步读入/写出（AIO），或者处于“IO 相关的固定”状态。
否则会出现：
读 IO 还没完成，你把 frame 换了：IO 完成后 DMA/拷贝写到“旧地址”，写飞
写 IO 正在刷盘，你释放/复用
frame：刷出去的数据不是你想要的页内容，导致磁盘损坏或页校验失败
    */
    return (false);
  }
  ...
  if (buf_page_get_state(bpage) == BUF_BLOCK_FILE_PAGE
    ...
    buf_block_t *evict_block = reinterpret_cast<buf_block_t *>(bpage);

    if (evict_block->frame != nullptr) {
      fil_read_cache::put(
          bpage->id,
          bpage->size.physical(),
          evict_block->frame);
      /* 计数和日志完全委托给 fil_read_cache 内部；这里不再打印。
         否则热淘汰路径上每次 evict 一条 stderr 会严重拖慢 O_DIRECT 扫表。 */
    }
  }
