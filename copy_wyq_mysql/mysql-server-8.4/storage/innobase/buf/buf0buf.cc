...
struct Buf_fetch {
 ...
 public:
  ...
  bool m_found_in_lookup{false};
  bool m_started_read{false};
...
dberr_t Buf_fetch_normal::get(buf_block_t *&block) noexcept {
  ...
  m_found_in_lookup = false;
  m_started_read = false;
  for (;;) {
...
buf_block_t *Buf_fetch<T>::single_page() {
  ...
  /* Cache-source monitor: count only lookups that were already resident in
  buffer pool and not brought in by this fetch path. */
