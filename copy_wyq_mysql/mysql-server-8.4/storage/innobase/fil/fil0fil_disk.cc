 ...
 *  For read operations: check cache first, on miss read from disk and populate
 *  cache. For write operations: invalidate the cached page. */
...
/** Maximum number of pages in the read-through cache (default ~16K pages) */
constexpr size_t FIL_READ_CACHE_MAX_PAGES = 16384;

/** The read-through cache: page_id -> (data, len) */
...
/** Initialize the read-through cache. Always clears any existing entries to
ensure a clean state on restart (avoids serving stale data from previous run
or from recovery phase). */
void init() {
  std::lock_guard<std::mutex> lock(s_cache_mutex);
  /* Clear any existing cache entries (handles re-init or restart scenarios) */
  for (auto &p : s_cache) {
...
/** Close and free the read-through cache */
void close() {
  std::lock_guard<std::mutex> lock(s_cache_mutex);
  if (!s_initialized) {
    return;
...
bool get(const page_id_t &page_id, ulint len, void *buf) {
  ...
  std::cout << "get page from cache: " << page_id.space() << "," << page_id.page_no() << std::endl;
  return true;
}

/** Put page into cache after reading from disk */
void put(const page_id_t &page_id, ulint len, const void *buf) {
  ...
  while (s_cache.size() >= FIL_READ_CACHE_MAX_PAGES && !s_lru_list.empty()) {
    evict_one();
  }
  byte *data = static_cast<byte *>(ut::malloc_withkey(UT_NEW_THIS_FILE_PSI_KEY,
                                                       len));
  if (data == nullptr) {
    return;
  }
  ...
  s_lru_list.push_back(page_id);
  std::cout << "put page into cache: " << page_id.space() << ","
...
void invalidate(const page_id_t &page_id) {
  ...
  std::cout << "invalidate page from cache: " << page_id.space() << ","
            << page_id.page_no() << std::endl;
}
...
static inline dberr_t pread_pwrite_file(const char *filename, bool is_write,
                                        ulint byte_offset, ulint len,
                                        void *buf) {
  int fd = ::open(filename, is_write ? (O_CREAT | O_RDWR) : O_RDONLY, 0644);
  if (fd < 0) return DB_IO_ERROR;

  ssize_t n = is_write ? ::pwrite(fd, buf, (size_t)len, (off_t)byte_offset)
                       : ::pread(fd, buf, (size_t)len, (off_t)byte_offset);
...
dberr_t fil_io(const IORequest &type, bool sync, const page_id_t &page_id,
  ...
  /* Check hijack path first - when it exists, we read from custom files
  instead of InnoDB tablespace. Do NOT use read-through cache for hijack path:
  those files may have inconsistent data (e.g. from different run) causing
  trx_id sanity check failures. Cache only the real InnoDB path. */
  namespace fs = std::filesystem;
  std::string root_dir = "/workspace/mysql_install/runtime_data/";
  std::string filename = root_dir + std::to_string(page_id.space()) + "_" +
                         std::to_string(page_id.page_no()) + ".page";
  const bool use_hijack = sync && file_exists(filename.c_str());

#ifndef UNIV_HOTBACKUP
  if (!use_hijack) {
    /* Read-through cache: for sync full-page reads from real InnoDB path,
    check cache first. Works during recovery too - we invalidate on write. */
    if (type.is_read() && sync && byte_offset == 0 &&
  ...
  if (use_hijack) {
    dberr_t e = pread_pwrite_file(filename.c_str(), type.is_write(),
                                  byte_offset, len, buf);
    std::cout << "fil_io hijacked for (" << page_id.space() << "," << page_id.page_no() << ")"
              << ", byte_offset: " << byte_offset << ", len: " << len
              << ", sync: " << sync
              << ", type: " << (type.is_read() ? "read" : "write") << std::endl;
    /* Do NOT cache hijack path data - it may be inconsistent with DB state */
    return e;
  }
  ...
  auto const err = shard->do_io(type, sync, page_id, page_size, byte_offset,
                                len, buf, message);
#ifndef UNIV_HOTBACKUP
  /* Populate read-through cache on successful disk read (real InnoDB path) */
