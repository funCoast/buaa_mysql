...
#ifndef UNIV_HOTBACKUP
namespace fil_cache_monitor {

namespace {

static bool s_enabled = false;
static bool s_trace = false;
static std::atomic<uint64_t> s_buf_hit{0};
static std::atomic<uint64_t> s_runtime_hit_sync{0};
static std::atomic<uint64_t> s_runtime_hit_async{0};
static std::atomic<uint64_t> s_disk_read_sync{0};
static std::atomic<uint64_t> s_disk_read_async{0};
static uint64_t s_cost_buf_ns = 0;
static uint64_t s_cost_runtime_ns = 0;
static uint64_t s_cost_disk_ns = 0;

static bool env_truthy(const char *key, bool dflt) {
  const char *v = std::getenv(key);
  if (!v || !*v) return dflt;
  return !(v[0] == '0' && v[1] == '\0');
}

static uint64_t env_u64_or(const char *key, uint64_t dflt) {
  const char *v = std::getenv(key);
  if (!v || !*v) {
    return dflt;
  }
...
static uint64_t env_u64(const char *key, uint64_t dflt) {
  const char *v = std::getenv(key);
  if (!v || !*v) {
    return dflt;
  }
...
static void dump_stats_to_err(const char *tag) {
  if (!s_enabled) {
    return;
  }

  const auto buf = s_buf_hit.load(std::memory_order_relaxed);
  const auto runtime_sync = s_runtime_hit_sync.load(std::memory_order_relaxed);
  const auto runtime_async =
      s_runtime_hit_async.load(std::memory_order_relaxed);
  const auto disk_sync = s_disk_read_sync.load(std::memory_order_relaxed);
  const auto disk_async = s_disk_read_async.load(std::memory_order_relaxed);
  const auto runtime_total = runtime_sync + runtime_async;
  const auto disk_total = disk_sync + disk_async;
  const auto logical_total = buf + runtime_sync + disk_sync;
  const auto all_total = logical_total + runtime_async + disk_async;
  const auto est_logical_ns =
      buf * s_cost_buf_ns + runtime_sync * s_cost_runtime_ns +
      disk_sync * s_cost_disk_ns;
  const auto est_all_ns = est_logical_ns +
                          runtime_async * s_cost_runtime_ns +
                          disk_async * s_cost_disk_ns;

  std::cerr << "[fil_cache_monitor][stats]"
            << " tag=" << tag
            << " buf_hit=" << buf
            << " runtime_hit=" << runtime_total
            << " runtime_hit_sync=" << runtime_sync
            << " runtime_hit_async=" << runtime_async
            << " disk_read=" << disk_total
            << " disk_read_sync=" << disk_sync
            << " disk_read_async=" << disk_async
            << " logical_total=" << logical_total
            << " all_total=" << all_total
            << std::endl;

  std::cerr << "[fil_cache_monitor][estimate_ns]"
            << " tag=" << tag
            << " cost_buf=" << s_cost_buf_ns
            << " cost_runtime=" << s_cost_runtime_ns
            << " cost_disk=" << s_cost_disk_ns
            << " logical_total_ns=" << est_logical_ns
            << " all_total_ns=" << est_all_ns
            << std::endl;
}

static void maybe_trace(const char *src, const page_id_t &page_id) {
  if (!s_enabled || !s_trace) {
    return;
  }
  std::cerr << "[fil_cache_monitor][" << src << "] space="
            << page_id.space() << " page=" << page_id.page_no()
            << std::endl;
}
...
void init() {
  s_enabled = env_truthy("FIL_CACHE_MONITOR_ENABLE", false);
  s_trace = env_truthy("FIL_CACHE_MONITOR_TRACE", false);
  s_buf_hit.store(0, std::memory_order_relaxed);
  s_runtime_hit_sync.store(0, std::memory_order_relaxed);
  s_runtime_hit_async.store(0, std::memory_order_relaxed);
  s_disk_read_sync.store(0, std::memory_order_relaxed);
  s_disk_read_async.store(0, std::memory_order_relaxed);
  s_cost_buf_ns = env_u64("FIL_CACHE_MONITOR_COST_BUF_NS", 0);
  s_cost_runtime_ns = env_u64("FIL_CACHE_MONITOR_COST_RUNTIME_NS", 0);
  s_cost_disk_ns = env_u64("FIL_CACHE_MONITOR_COST_DISK_NS", 0);
  std::cerr << "[fil_cache_monitor] init enabled=" << (s_enabled ? 1 : 0)
            << " trace=" << (s_trace ? 1 : 0)
            << " cost_buf_ns=" << s_cost_buf_ns
            << " cost_runtime_ns=" << s_cost_runtime_ns
            << " cost_disk_ns=" << s_cost_disk_ns << std::endl;
}

bool enabled() { return s_enabled; }

bool is_cacheable_page(const page_id_t &page_id) {
  return page_id.space() != TRX_SYS_SPACE &&
         !fsp_is_system_temporary(page_id.space()) &&
         !fsp_is_undo_tablespace(page_id.space());
}

void record_buf_hit(const page_id_t &page_id) {
  if (!s_enabled || !is_cacheable_page(page_id)) {
    return;
  }
  s_buf_hit.fetch_add(1, std::memory_order_relaxed);
  maybe_trace("BUF", page_id);
}

void record_runtime_hit(const page_id_t &page_id, bool sync) {
  if (!s_enabled || !is_cacheable_page(page_id)) {
    return;
  }
  (sync ? s_runtime_hit_sync : s_runtime_hit_async)
      .fetch_add(1, std::memory_order_relaxed);
  maybe_trace(sync ? "RUNTIME_SYNC" : "RUNTIME_ASYNC", page_id);
}

void record_disk_read(const page_id_t &page_id, bool sync) {
  if (!s_enabled || !is_cacheable_page(page_id)) {
    return;
  }
  (sync ? s_disk_read_sync : s_disk_read_async)
      .fetch_add(1, std::memory_order_relaxed);
  maybe_trace(sync ? "DISK_SYNC" : "DISK_ASYNC", page_id);
}

}  // namespace fil_cache_monitor

/** Read-through cache: a layer between memory (buffer pool) and disk.
 *
 *  优先使用分布式 DSM runtime 作为后端（通过 dsm_bridge 连接到外部 simulator
 *  上的共享内存）。若 DSM 不可用（比如未启动 simulator / export_client），
 *  则自动 fallback 到原有的进程内 LRU 缓存，保证 mysqld 在任何情况下都能启动。
 *
 *  端到端测试所需的环境：
 *    1) `mpirun -np 4 ./simulator`           （终端 1）
 *    2) `mpirun -np 4 ./export_client`       （终端 2，跑完即可）
 *    3) 启动 mysqld，init() 会自动 mmap
 *       /dev/shm/virtual_node0/obmm_shmdev{1,2,3} 并作为 DSM 缓存后端。
 */
...
namespace fil_read_cache {

/** Hash functor for page_id_t in std::unordered_map */
struct Page_id_hash {
...
/** Cached page entry: (data pointer, length) */
struct Cache_entry {
  byte *data;
  ulint len;

  Cache_entry() : data(nullptr), len(0) {}
  Cache_entry(byte *d, ulint l) : data(d), len(l) {}
};

/** Default maximum number of pages in the fallback in-process read-through
cache. Can be overridden by FIL_READ_CACHE_MAX_PAGES. */
constexpr size_t FIL_READ_CACHE_MAX_PAGES_DEFAULT = 16384;
static size_t s_fallback_max_pages = FIL_READ_CACHE_MAX_PAGES_DEFAULT;

/** Fallback 缓存（DSM 不可用时使用）：page_id -> (data, len) */
static std::unordered_map<page_id_t, Cache_entry, Page_id_hash> s_cache;
/** LRU list for eviction: front = oldest */
static std::list<page_id_t> s_lru_list;
/** Mutex protecting the fallback cache */
static std::mutex s_cache_mutex;
/** Whether the read-through cache subsystem is initialized */
static bool s_initialized = false;
/** Whether DSM backend is active (true => skip in-memory cache entirely) */
static bool s_dsm_active = false;

/*===========================================================================
 * Runtime switches (read from env at init()):
 *   FIL_READ_CACHE_ENABLE  - 0 to completely bypass this whole layer (pure
 *                            InnoDB, used as the "no-cache" control group).
 *                            default: 1
 *   FIL_READ_CACHE_TRACE   - 0 quiet (only numeric counters), 1 log every
 *                            get/put/invalidate to stderr. Logging is
 *                            expensive on hot paths and MUST be off for
...
static std::atomic<uint64_t> s_ctr_get_hit{0};
static std::atomic<uint64_t> s_ctr_get_miss{0};
static std::atomic<uint64_t> s_ctr_put{0};
static std::atomic<uint64_t> s_ctr_invalidate{0};

/** 时间分解用的纳秒累计器（多线程累加，打印时 / ops 得到平均单次耗时）。
 *    ns_get        - fil_read_cache::get 整个调用
 *    ns_put        - fil_read_cache::put 整个调用
 *    ns_inv        - fil_read_cache::invalidate 整个调用
 *    ns_doio_read  - 真正 fallthrough 到 shard->do_io 的 READ 部分（sync + async 提交）
 *    cnt_doio_read - do_io(read) 被调用次数（与 ns_doio_read 配对算单次时延）
 */
static std::atomic<uint64_t> s_ns_get{0};
static std::atomic<uint64_t> s_ns_put{0};
static std::atomic<uint64_t> s_ns_inv{0};
static std::atomic<uint64_t> s_ns_doio_read{0};
static std::atomic<uint64_t> s_cnt_doio_read{0};
...
/** 外部触发：后台线程每 5ms 检查 /tmp/fil_read_cache_dump 是否存在；存在就
    dump 一次并 unlink。bench 脚本在查询前后各 `touch /tmp/fil_read_cache_dump`
    就能拿到窗口 delta。不用信号，避免 MySQL 自己的 SIGUSR1 处理冲突。*/
static std::atomic<bool> s_dumper_run{false};
static std::thread s_dumper_thr;

static void dump_stats_to_err(const char *tag);

static void dumper_loop() {
  const char *trig = "/tmp/fil_read_cache_dump";
  while (s_dumper_run.load(std::memory_order_relaxed)) {
    struct stat st;
    if (::stat(trig, &st) == 0) {
      dump_stats_to_err("window");
      ::unlink(trig);
    }
...
static void dump_stats_to_err(const char *tag) {
  auto gh = s_ctr_get_hit.load(std::memory_order_relaxed);
  auto gm = s_ctr_get_miss.load(std::memory_order_relaxed);
  auto pt = s_ctr_put.load(std::memory_order_relaxed);
  auto iv = s_ctr_invalidate.load(std::memory_order_relaxed);
  auto ns_g = s_ns_get.load(std::memory_order_relaxed);
  auto ns_p = s_ns_put.load(std::memory_order_relaxed);
  auto ns_i = s_ns_inv.load(std::memory_order_relaxed);
  auto ns_d = s_ns_doio_read.load(std::memory_order_relaxed);
  auto cd   = s_cnt_doio_read.load(std::memory_order_relaxed);

  std::cerr << "[fil_read_cache][stats]"
            << " tag=" << tag
            << " backend="
            << (s_layer_enabled ? (s_dsm_active ? "DSM" : "mem-fallback")
                                : "disabled")
            << " get_hit=" << gh
            << " get_miss=" << gm
            << " put=" << pt
            << " invalidate=" << iv
            << std::endl;
  std::cerr << "[fil_read_cache][timing_ns]"
            << " get=" << ns_g << "(n=" << (gh + gm) << ")"
            << " put=" << ns_p << "(n=" << pt << ")"
            << " invalidate=" << ns_i << "(n=" << iv << ")"
            << " doio_read=" << ns_d << "(n=" << cd << ")"
            << std::endl;
  fil_cache_monitor::dump_stats_to_err(tag);
}

/** Initialize the read-through cache. 优先尝试启用 DSM 后端，失败则回退到内存
cache。无论哪条路径都会清空状态以保证重启后一致性（防止从上次运行或 recovery
阶段残留的脏缓存读到脏数据）。 */
void init() {
  ...
  fil_cache_monitor::init();

  s_ctr_get_hit.store(0);
  s_ctr_get_miss.store(0);
  s_ctr_put.store(0);
  s_ctr_invalidate.store(0);
  s_ns_get.store(0);
  s_ns_put.store(0);
  s_ns_inv.store(0);
  s_ns_doio_read.store(0);
  s_cnt_doio_read.store(0);

  /* 启动后台 dumper：监听 /tmp/fil_read_cache_dump 文件创建事件 → dump stats */
  if (!s_dumper_run.load(std::memory_order_relaxed)) {
    s_dumper_run.store(true, std::memory_order_relaxed);
    s_dumper_thr = std::thread(dumper_loop);
  }

  if (!s_layer_enabled) {
    std::cerr << "[fil_read_cache] init: layer DISABLED "
                 "(FIL_READ_CACHE_ENABLE=0) -> pure InnoDB path" << std::endl;
    s_dsm_active = false;
  ...
  /* 尝试启动 DSM 后端。setup() 幂等，内部有锁。 */
  s_dsm_active = dsm_bridge::setup();

  std::lock_guard<std::mutex> lock(s_cache_mutex);
  for (auto &p : s_cache) {
    if (p.second.data != nullptr) {
      ut::free(p.second.data);
    }
  }
  s_cache.clear();
  s_lru_list.clear();
...
/** Close and free the read-through cache (both backends) */
void close() {
  dump_stats_to_err("close");

  if (s_dumper_run.exchange(false, std::memory_order_relaxed)) {
    if (s_dumper_thr.joinable()) {
      s_dumper_thr.join();
    }
  }

  /* Tear down DSM first so that no background put/get races with munmap. */
  if (s_dsm_active) {
    dsm_bridge::teardown();
    s_dsm_active = false;
  }
...
/** Evict oldest entry when fallback cache is full */
static void evict_one() {
  if (s_lru_list.empty()) {
    return;
  }
  page_id_t victim = s_lru_list.front();
  s_lru_list.pop_front();
  auto it = s_cache.find(victim);
  if (it != s_cache.end()) {
    if (it->second.data != nullptr) {
      ut::free(it->second.data);
    }
    s_cache.erase(it);
  }
}

/** Try to get page from cache. Returns true if found and copied to buf */
bool get(const page_id_t &page_id, ulint len, void *buf) {
  if (!s_layer_enabled) {
    return false;
  }
  Timer __t;
  ...
  if (s_dsm_active) {
    if (dsm_bridge::read_page(page_id.space(), page_id.page_no(), buf, len)) {
      ...
      if (s_trace) {
        std::cerr << "[fil_read_cache][dsm] HIT  " << page_id.space() << ","
                  << page_id.page_no() << std::endl;
      }
      return true;
    }
    s_ctr_get_miss.fetch_add(1, std::memory_order_relaxed);
    return false;
  }
  ...
  auto it = s_cache.find(page_id);
  if (it == s_cache.end() || it->second.len != len) {
    s_ctr_get_miss.fetch_add(1, std::memory_order_relaxed);
    return false;
  }
  memcpy(buf, it->second.data, len);
  s_lru_list.remove(page_id);
  s_lru_list.push_back(page_id);
  s_ctr_get_hit.fetch_add(1, std::memory_order_relaxed);
  if (s_trace) {
    std::cerr << "[fil_read_cache][mem] HIT  " << page_id.space() << ","
              << page_id.page_no() << std::endl;
  }
  return true;
}

/** Put page into cache after reading from disk or on LRU eviction */
void put(const page_id_t &page_id, ulint len, const void *buf) {
  ...
  if (s_dsm_active) {
    if (dsm_bridge::write_page(page_id.space(), page_id.page_no(), buf, len)) {
      ...
      if (s_trace) {
        std::cerr << "[fil_read_cache][dsm] PUT  " << page_id.space() << ","
                  << page_id.page_no() << std::endl;
      }
    }
    return;
  ...
  while (s_cache.size() >= s_fallback_max_pages && !s_lru_list.empty()) {
    evict_one();
  }
  ...
  s_cache[page_id] = Cache_entry(data, len);
  s_lru_list.push_back(page_id);
  s_ctr_put.fetch_add(1, std::memory_order_relaxed);
  if (s_trace) {
    std::cerr << "[fil_read_cache][mem] PUT  " << page_id.space() << ","
              << page_id.page_no() << std::endl;
  }
}
...
void invalidate(const page_id_t &page_id) {
  ...
  if (s_dsm_active) {
    ...
    if (s_trace) {
      std::cerr << "[fil_read_cache][dsm] INV  " << page_id.space() << ","
                << page_id.page_no() << std::endl;
    }
    return;
  }
  ...
  s_ctr_invalidate.fetch_add(1, std::memory_order_relaxed);
  if (s_trace) {
    std::cerr << "[fil_read_cache][mem] INV  " << page_id.space() << ","
              << page_id.page_no() << std::endl;
  }
}
...
void fil_init(ulint max_n_open) {
  ...
  ib::info() << "fil_init: starting fil subsystem"
  ...
  switch (file->space->purpose) {
    ...
    case FIL_TYPE_TABLESPACE:
      ...
      if (m2 != nullptr) {
        ...
        /* 成功 async 读完成 -> 也放进 L2 read cache（DSM 或 fallback）。
           这是"冷读装缓存"路径在 async 情形下的补齐。写 / 压缩页
           不处理；sync read 在 fil_io 末尾已经 put 过。 */
        if (type.is_read() && !bpage->is_dirty() &&
          ...
          if (block->frame != nullptr) {
            fil_read_cache::put(bpage->id, bpage->size.physical(),
                                block->frame);
          }
        }
...
#ifndef UNIV_HOTBACKUP
  /* --------------------------------------------------------------------
   * Read-through cache check (sync + async).
   * For async reads we also want to serve from cache to make benchmarks
   * reflect L2 benefit on BP-miss dominated scans; we then synthesise an
   * I/O completion so the caller sees the same state as a disk read.
   * -------------------------------------------------------------------- */
  if (type.is_read() && byte_offset == 0 &&
      fil_read_cache::get(page_id, len, buf)) {
    if (fil_read_cache::s_dsm_active) {
      fil_cache_monitor::record_runtime_hit(page_id, sync);
    }
    if (!sync) {
      /* Synthetic AIO completion: flip io_fix / buf state as a real
      completion would, without touching the disk. Responsibility is still
      with the caller at this point (we haven't reached the
      release_io_responsibility() call below), so buf_page_io_complete
      sees the expected state. */
      auto bpage = static_cast<buf_page_t *>(message);
      IORequest completion_type = type;
      buf_page_io_complete(bpage, false, &completion_type, nullptr);
    }
    return DB_SUCCESS;
  }

  /* On write: invalidate the page from read-through cache */
  if (type.is_write()) {
    fil_read_cache::invalidate(page_id);
  }
#endif /* !UNIV_HOTBACKUP */
...
#ifndef UNIV_HOTBACKUP
  /* 计时并记日志：这里是"真正进到 shard->do_io"的 fallthrough 路径。
  ...
  if (__is_read) {
    ...
    if (fil_read_cache::s_trace) {
      std::cerr << "[fil_io][DISK] " << (sync ? "sync" : "async")
                << " space=" << page_id.space()
                << " page=" << page_id.page_no() << std::endl;
    }
  }
  /* Populate read-through cache on successful SYNC disk read; for async,
  buffer is filled later in the completion callback (hooked in fil_aio_wait). */
  if (err == DB_SUCCESS && type.is_read() && sync && byte_offset == 0) {
    fil_read_cache::put(page_id, len, buf);
  }
#endif /* !UNIV_HOTBACKUP */
#ifdef UNIV_DEBUG
...
// ib::info() << "fil_io:" << " space_id=" << page_id.space()
//            << " page_no=" << page_id.page_no()
//            << " byte_offset=" << byte_offset
//            << " len=" << len
//            << " sync=" << sync
//            << " type=" << (type.is_read() ? "read" : "write");
