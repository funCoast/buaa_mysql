...
static void wp_set(int uffd, void *addr, size_t len, int enable) {
...
// 首次写（WP fault）时
static void hook_first_write_wp(void *page_addr) {
    fprintf(stderr, "[WP]      page=%p first-write trapped\n", page_addr);
    fflush(stderr);
}
...
static void *fault_handler(void *arg) {
    ...
    for (;;) {
        ...
        if (flags & UFFD_PAGEFAULT_FLAG_WP) {
            hook_first_write_wp(page_addr);
            // 放行这次写：解除写保护
            wp_set(c->uffd, page_addr, c->pagesz, 0);
        ...
        // 2) MINOR fault：invalidate 后再次访问（页在 page cache，PTE 不在）
...
static void invalidate_page(ctx_t *c, void *any_addr_in_page) {
    ...
    // re-arm 写保护：保证下一次写还能触发 WP
    wp_set(c->uffd, page, c->pagesz, 1);
...
int main() {
    ...
    if (!(ua.features & UFFD_FEATURE_PAGEFAULT_FLAG_WP) ||
        !(ua.features & UFFD_FEATURE_WP_HUGETLBFS_SHMEM)) {
        fprintf(stderr, "Need write-protect on shmem: UFFD_FEATURE_WP_HUGETLBFS_SHMEM (Linux 5.19+).\n");
        return 2;
    }
    ...
    // 先把整段都写保护上（读不受影响；首次写会 WP fault）
    wp_set(c.uffd, c.base, c.len, 1);
