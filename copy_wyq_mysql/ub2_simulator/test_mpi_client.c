...
int main(int argc, char *argv[]) {
    ...
    char target_node_name[256];
    sprintf(target_node_name, "%d", mpi_rank);
    setenv("OBMM_TARGET_NODE", target_node_name, 1);
    MPI_Finalize();
    ...
    if (mpi_rank == 0) {
        ...
        printf("[User Process 0] Connecting to simulator node 0 to export memory...\n");
        ...
        // Process 1: Import memory from simulator node 0, use it on simulator node 1
        printf("[User Process 1] Waiting for address from process 0...\n");
        ...
        printf("[User Process 1] Importing memory at address 0x%lx on simulator node 1...\n", addr);
        ...
        // The symlink path should be: /dev/shm/virtual_node1/obmm_shmdev{import_memid}
        char shm_path[256];
        snprintf(shm_path, sizeof(shm_path), "/dev/shm/virtual_node1/obmm_shmdev%ld", import_memid);
        ...
        // Unimport
        printf("[User Process 1] Unimporting memory (mem_id=%ld)...\n", import_memid);
        int ret = obmm_unimport(import_memid, 0);
        if (ret == 0) {
            printf("[User Process 1] Successfully unimported\n");
        } else {
            fprintf(stderr, "[User Process 1] Failed to unimport\n");
        }
